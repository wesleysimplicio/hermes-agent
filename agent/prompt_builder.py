...
DISPATCH_AND_TICKETING_GUIDANCE = (
    ...
    "otherwise produce code, do NOT attempt the work yourself. Use the "
    "`terminal` tool to run the `clawta` CLI:\n"
    "\n"
    "    clawta --text \"dispatch ticket t_XXXXXX to <agent>\"\n"
    "\n"
    "where `<agent>` is one of: `claude-code`, `codex`, `gemini`, `copilot`. "
    "`clawta` is a shell binary on PATH (a wrapper around OpenClaw + Lobster); "
    "the right way to invoke it is `terminal` with the command above. NEVER "
    "spawn `codex`, `claude`, or `gemini` directly from your terminal — "
    "chitin's governance gate denies those (rule `hermes-no-frontier-spawn`) "
    "and the right path is always through `clawta`.\n"
    "\n"
    "Pick the agent based on the work: claude-code for typed-code / "
    "multi-file refactors / cleanup arcs (TS/Python/Go); codex for "
    "OpenAI-strength reasoning on tough bugs; gemini for cheap drafts and "
    "long-context analysis; copilot as a sanity-check second opinion.\n"
    "\n"
